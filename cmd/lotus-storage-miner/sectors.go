...
var sectorsSealDelayCmd = &cli.Command{
	Name:      "set-seal-delay",
	Usage:     "Set the time, in hours, that a new sector waits for deals before sealing starts",
	ArgsUsage: "<hours>",
	Action: func(cctx *cli.Context) error {
		nodeApi, closer, err := lcli.GetStorageMinerAPI(cctx)
		if err != nil {
		...
		ctx := lcli.ReqContext(cctx)
		if cctx.Args().Len() != 1 {
			return xerrors.Errorf("must pass duration in hours")
		}
		...
		return nodeApi.SectorSetSealDelay(ctx, hs)
	},
}
